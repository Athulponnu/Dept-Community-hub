...
import { GraduationCap, LogOut, BarChart3, Users, Award, TrendingUp, Loader2, Plus, Copy, Check, BookOpen, Trash2, ArrowLeft, FileText, Upload, File, Calendar, Heart } from "lucide-react";
...
const TeacherDashboard = ({ onLogout }: TeacherDashboardProps) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: teacher } = await supabase
        .from("teachers").select("name, bud_code").eq("id", user.id).maybeSingle();
      if (teacher) { setTeacherName(teacher.name); setBudCode(teacher.bud_code); }

      const { data: subjectsData } = await supabase
        .from("subjects").select("id, name, icon, join_code, teacher_id").eq("teacher_id", user.id);
  ...
  const handleCreateSubject = async () => {
    ...
    try {
      ...
      if (error) { setCreateError("Failed to create subject. Try again."); return; }
            ...
            <Card className="border-primary/30">
              ...
              <CardContent className="space-y-4">
                <div className="flex gap-2">
                  <Button size="sm" variant={noteType === "text" ? "default" : "outline"} onClick={() => setNoteType("text")} className="gap-1.5">
                    <FileText className="w-3.5 h-3.5" /> Text Note
                  </Button>
                  <Button size="sm" variant={noteType === "file" ? "default" : "outline"} onClick={() => setNoteType("file")} className="gap-1.5">
                ...
                {noteType === "text" ? (
                  <div className="space-y-2">
                    <Label>Content</Label>
                    <Textarea placeholder="Write your note content here..." value={noteContent} onChange={(e) => setNoteContent(e.target.value)} rows={5} />
                  </div>
                ) : (
                  <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Button onClick={handleAddNote} disabled={addingNote || !noteTitle.trim() || (noteType === "file" && !noteFile)}>
                    {addingNote ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Plus className="w-4 h-4 mr-2" />}
                    Add Note
                  </Button>
          ...
          {notesLoading ? (
            ...
            <Card className="border-dashed">
              <CardContent className="p-12 text-center">
                ...
                <p className="text-muted-foreground text-sm">Add your first note for this subject.</p>
              </CardContent>
            </Card>
          ) : (
                    ...
                    <div className="flex items-start gap-3 flex-1">
                      ...
                      <div className="flex-1">
                        ...
                        {note.note_type === "text" && note.content && (
                          <p className="text-sm text-muted-foreground mt-1.5 line-clamp-2">{note.content}</p>
                        )}
            ...
            <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    ...
                    {budCode ? (
                      <div className="flex items-center gap-3">
                        ...
                        <Button variant="ghost" size="sm" onClick={handleGenerateBudCode} className="text-muted-foreground text-xs">
                          Regenerate
                        </Button>
                      </div>
                    ) : (
                      <Button onClick={handleGenerateBudCode} className="gap-2">
                        <Plus className="w-4 h-4" /> Generate Bud Code
                      </Button>
                    )}
                  </div>
                </CardContent>
              ...
              {buds.length === 0 ? (
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {buds.map((bud) => (
                    <Card key={bud.id} className="group hover:border-primary/30 hover:shadow-md transition-all">
                      <CardContent className="p-5">
                        <div className="flex items-start justify-between mb-3">
                          ...
                          <Button size="sm" variant="ghost"
                            ...
                            onClick={() => handleRemoveBud(bud.id)}>
                            <Trash2 className="w-4 h-4" />
                          </Button>
                        </div>
                        <div className="flex items-center gap-2">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {ICONS.map((icon) => (
                          <button key={icon} onClick={() => setNewSubjectIcon(icon)}
                            className={`w-9 h-9 text-xl rounded-lg border-2 transition-all ${newSubjectIcon === icon ? "border-primary bg-primary/10" : "border-border hover:border-primary/40"}`}>
                            {icon}
                          </button>
                        ))}
                      </div>
                    ...
                    <div className="space-y-2">
                      <Label>Join Code <span className="text-muted-foreground font-normal">(optional — auto-generated if empty)</span></Label>
                      <Input placeholder="e.g. DS101" value={newSubjectCode} onChange={(e) => setNewSubjectCode(e.target.value.toUpperCase())}
                        className="font-mono tracking-widest uppercase max-w-[160px]" maxLength={8} />
                    </div>
              ...
              {subjects.length === 0 ? (
                <Card className="border-dashed">
                  <CardContent className="p-12 text-center">
                    ...
                    <p className="text-muted-foreground text-sm">Create your first subject to get started.</p>
                  </CardContent>
                </Card>
              ) : (
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <CardContent className="p-5">
                    ...
                    <div className="flex gap-1.5 mt-3 flex-wrap">
                      {student.joinedGroups.map((gId) => {
                        ...
                        return group ? <Badge key={gId} variant="outline" className="text-xs font-normal">{group.name}</Badge> : null;
                      })}
                    </div>
                  </CardContent>
                </Card>
                  ...
                  {[...students].sort((a, b) => b.contributions - a.contributions).map((student, idx) => (
                    <div key={student.id} className="flex items-center gap-4 p-3 rounded-xl bg-muted/40 hover:bg-muted/70 transition-colors cursor-pointer" onClick={() => setSelectedStudent(student)}>
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-muted-foreground">{student.joinedGroups.length} groups joined</p>
                      </div>
